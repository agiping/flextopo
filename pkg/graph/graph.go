...
func (g *FlexTopoGraph) BuildCPUNodes(cpuInfos []utils.CPUInfo) {
	...
	for _, cpuInfo := range cpuInfos {
		...
		coreGroupNode.Attributes["nodeID"] = fmt.Sprintf("%d", cpuInfo.NumaNodeID)
		coreGroupNode.Attributes["groupIndex"] = fmt.Sprintf("%d", cpuInfo.CoreID/g.CoreGroupSize)
...
func (g *FlexTopoGraph) NewGPUNode(index int, uuid, name string, memoryTotal int) *Node {
	...
	gpuNode := &Node{
		...
		Attributes: map[string]string{
			"uuid":        uuid,
			"name":        name,
			"memoryTotal": fmt.Sprintf("%d", memoryTotal),
			"status":      "free",
		},
	}
	return gpuNode
